...
function initMapLight() {
  ...
  if (mapObjects !== undefined && mapObjects.length) {
    ...
    if (geoObjects.length === 1) {
      ...
      var boundsLat = bounds[0][0];
      var boundsLng = bounds[0][1];
      map.setCenter([boundsLat, boundsLng], 12, {
        checkZoomRange: true
      });
    } else {
...
function addMarker(lat, lng, geo) {
  ...
  var href = geo.href;
  var title = geo.title;
  var balloonContent = '<div class="object__mapcard object__mapcard--light">';
